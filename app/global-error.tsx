...
// Define Open Sans font with specific subsets and weights
...
export default function GlobalError({ error, reset }: { error: Error; reset: () => void }) {
  ...
  // Attempt to recover by trying to re-render the segment after a short delay
  const retry = () => setTimeout(reset, 100);

  // Hard reset by reloading the page after a short delay
  const reload = () => setTimeout(() => location.reload(), 100);

  // CSS class names for animation elements
  const zeroClassName =
  ...
  const crossClassName = `${barClassName.replaceAll(' ', ' before:').trim()} ${barClassName.replaceAll(' ', ' after:').trim()}`;
    ...
    <div className={openSans.className}>
      ...
      >
        <p className="px-6 z-10">
          Oops! The app encountered an issue... <br />
          Please report it to{' '}
          <Link target="_blank" href={`mailto:${EMAIL}?subject=Unexpected error on ${window.location}`}>
            {EMAIL}
          </Link>
        </p>

...
Number.prototype.toLocaleCurrency = function (currency = 'EUR') {
  let value = Intl.NumberFormat(navigator.language, { style: 'currency', currency }).format(Number(this));
  if (this && /^(?:\D*0(?:\.\d{1,2})?)\D*$/.test(value)) {
    const significantDigits = this.toPrecision(2);
    value = value.replace('00', parseFloat(significantDigits).toString().slice(2));
  }
  return value;
};
...
Number.prototype.toDecimalPlace = function (decimalPlace = 2, direction: RoundingDirection = 'up') {
  ...
  const roundedValue = direction === 'up' ? Math.ceil(Number(this) * multiplier) : Math.floor(Number(this) * multiplier);
